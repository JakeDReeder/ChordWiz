...
export const getAccidentNote = (
  semitones: number,
  mode = ACCIDENTAL_MODE.SHARP
) => {
 ...
 * @returns
 */
export const getPitchedNote = (
  frequency: number,
  mode = ACCIDENTAL_MODE.SHARP
): IPitchedNote => {
  const octave = Math.log2(frequency / A_FREQUENCY);
  const semitones = 12 * octave;
  const closestFrequency =
    A_FREQUENCY * Math.pow(2, (Math.round(semitones) * 100) / 1200);
